...
- レパートリー登録（パート / ボーカル / キー / 習熟度）
- ルーム作成（QRコード / 6桁コード共有、24時間で自動消滅）
- 3段階マッチング表示（Strict / Normal / Loose）
...
# 3. テストユーザーをSupabase Authに作成
bash supabase/seed-auth.sh

# 4. テストユーザーのprofilesレコードを作成するため再度リセット
npx supabase db reset
```

> **注意**: `seed-auth.sh` はAuth APIでユーザーを作成し、`seed.sql` がそのユーザーのprofilesレコードを挿入する。そのため手順3の後にもう一度 `db reset` が必要。
...
├── migrations/
│   └── 20260406000000_initial_schema.sql    # DBスキーマ（テーブル、RLS、インデックス）
├── seed.sql                                 # 初期データ（曲マスター50曲 + テストユーザーprofile）
└── seed-auth.sh                             # テストユーザーAuth作成スクリプト
```
...
| カラム | 値 | 説明 |
|--------|---|------|
| `part` | `guitar` / `banjo` / `mandolin` / `fiddle` / `bass` / `dobro` / `other` | 担当パート |
| `vocal` | `lead` / `harmony_high` / `harmony_low` / `none` | ボーカル担当 |
| `proficiency` | `ready` / `with_practice` / `learning` | 習熟度（いつでもOK / 要練習 / 挑戦中） |
| `preferred_keys` | text[] | 得意キー（複数指定可） |
...
`room_members` テーブルがSupabase Realtimeの購読対象として設定されている（`ALTER PUBLICATION supabase_realtime ADD TABLE room_members`）。
...
| `useRoom` | `src/hooks/use-room.ts` | ルーム作成・参加 | `createRoom`, `joinRoom` |
...
| `/` | ホーム | `useAuth` |
| `/repertoire` | 曲検索・レパートリー管理 | `useSongs`, `useRepertoire` |
| `/room/create` | ルーム作成（QRコード・コード表示） | `useRoom` |
| `/room/join` | ルーム参加（コード入力） | `useRoom` |
| `/room/[id]` | ルーム内マッチング画面 | `useRoomMembers`, `useMatching` |
